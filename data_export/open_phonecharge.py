...
csv_group = glob.glob('./phonecharge/*.csv')
for i in csv_group:
...
for i in csv_group:
    with open(i, encoding = 'utf-8') as csv_files:
        ...
        #print(len(tmp))
        ass = []
        ass.append(round(sum(morning)/5788800,4))
        ass.append(round(sum(afternoon)/5788800,4))
        ass.append(round(sum(evening)/5788800,4))
        ass.append(round(sum(night)/5788800,4))
